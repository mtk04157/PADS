...
Sub Main()
    ...
    Exit Sub
ErrorHandler:
    MsgBox "错误: " & Err.Description & vbCrLf & "行: " & Err.Source, vbCritical
End Sub
...
Sub SmartCompactLayout(PCBDoc As Object, selectedComps As Object)
    ...
    Dim i As Integer
    Dim comp As Object
    Dim specialComps() As String
    Dim isSpecial As Boolean
    Dim regularComps As New Collection
    Dim specialCompsList As New Collection
    ...
    For i = 0 To selectedComps.Count - 1
        ...
        '检查是否为特殊组件
        Dim j As Integer
        For j = LBound(specialComps) To UBound(specialComps)
            If InStr(comp.Name, specialComps(j)) > 0 Then
                specialCompsList.Add comp
                isSpecial = True
                Exit For
            End If
        Next j
    ...
    If specialCompsList.Count > 0 Then
        Call PlaceSpecialComponentsOutlineAligned(PCBDoc, specialCompsList)
    End If
...
Sub PlaceSpecialComponentsOutlineAligned(PCBDoc As Object, specialComps As Collection)
    ...
    For i = 2 To specialComps.Count
        ...
        '记录日志
        Debug.Print "特殊组件 " & comp.Name & " Outline对齐到: (" & x & ", " & y & ")"
    Next i
End Sub
...
Sub PlaceRegularComponentsCompact(PCBDoc As Object, regularComps As Collection)
    ...
    For i = 1 To regularComps.Count
        ...
        Debug.Print "普通组件 " & comp.Name & " 放置到: (" & comp.LocationX & ", " & comp.LocationY & ")"
    Next i
End Sub
...
Sub DrawStraightTrace(PCBDoc As Object, net As Object, x1 As Double, y1 As Double, x2 As Double, y2 As Double)
    ...
    Dim topLayer As String

    topLayer = "TOP"

    '先水平后竖直 (L形走线)
    If x1 <> x2 Then
        Call AddTraceSegment(PCBDoc, net, topLayer, x1, y1, x2, y1)
    End If

    If y1 <> y2 Then
        Call AddTraceSegment(PCBDoc, net, topLayer, x2, y1, x2, y2)
    End If
...
'添加走线段
Sub AddTraceSegment(PCBDoc As Object, net As Object, layer As String, x1 As Double, y1 As Double, x2 As Double, y2 As Double)
    On Error Resume Next

    Dim seg As Object
    Dim layerObj As Object

    Set layerObj = PCBDoc.LayerSet.Item(layer)
...
Function GetComponentWidth(comp As Object) As Double
    On Error Resume Next
    GetComponentWidth = Abs(comp.BBox.Xmax - comp.BBox.Xmin) + 0.1
    On Error GoTo 0
End Function
...
Function GetComponentHeight(comp As Object) As Double
    On Error Resume Next
    GetComponentHeight = Abs(comp.BBox.Ymax - comp.BBox.Ymin) + 0.1
    On Error GoTo 0
End Function
